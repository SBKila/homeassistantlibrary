...
void UT_LinkedList(void) {
  ...
  TEST_ASSERT_TRUE_MESSAGE(linkedList.append(&hAComponent1),
                           "append should return true");
  TEST_ASSERT_TRUE_MESSAGE(linkedList.append(&hAComponent2),
                           "append hAComponent2 should return true");
  TEST_ASSERT_TRUE_MESSAGE(linkedList.append(&hAComponent3),
                           "append hAComponent3 should return true");
  TEST_ASSERT_EQUAL_INT(3, linkedList.getSize());

  // test find by address
  TEST_MESSAGE("test find by address");
  TEST_ASSERT_EQUAL_INT_MESSAGE(1, linkedList.find(&hAComponent2, false),
                                "hAComponent2 should be found at position 1");
  TEST_ASSERT_EQUAL_INT_MESSAGE(2, linkedList.find(&hAComponent3, false),
                                "hAComponent3 should be found at position 2");
  TEST_ASSERT_EQUAL_INT_MESSAGE(0, linkedList.find(&hAComponent1, false),
                                "hAComponent1 should be found at position 0");
  TEST_ASSERT_EQUAL_INT_MESSAGE(-1, linkedList.find(&hAComponent4, false),
                                "hAComponent4 should not be found");
  TEST_ASSERT_EQUAL_INT_MESSAGE(-1, linkedList.find(&hAComponent10, false),
                                "hAComponent10 should not be found");

  // test find by contant
  TEST_MESSAGE("test find by contant");
  TEST_ASSERT_EQUAL_INT_MESSAGE(1, linkedList.find(&hAComponent2, true),
                                "hAComponent2 should be found at position 1");
  TEST_ASSERT_EQUAL_INT_MESSAGE(2, linkedList.find(&hAComponent3, true),
                                "hAComponent3 should be found at position 2");
  TEST_ASSERT_EQUAL_INT_MESSAGE(0, linkedList.find(&hAComponent1, true),
                                "hAComponent1 should be found at position 0");
  TEST_ASSERT_EQUAL_INT_MESSAGE(-1, linkedList.find(&hAComponent4, true),
                                "hAComponent4 should not be found");
  TEST_ASSERT_EQUAL_INT_MESSAGE(0, linkedList.find(&hAComponent10, true),
                                "hAComponent10 should be found at position 0");
  ...
  TEST_ASSERT_EQUAL_PTR_MESSAGE(&hAComponent2, item,
                                "position 1 should be hAComponent2");
  item = linkedList.get(2);
  TEST_ASSERT_NOT_NULL_MESSAGE(item, "one element should be at position 2");
  TEST_ASSERT_EQUAL_PTR_MESSAGE(&hAComponent3, item,
                                "position 2 should be hAComponent3");
  item = linkedList.get(0);
  TEST_ASSERT_NOT_NULL_MESSAGE(item, "one element should be at position 0");
  TEST_ASSERT_EQUAL_PTR_MESSAGE(&hAComponent1, item,
                                "position 0 should be hAComponent1");

  // test remove by address
  TEST_MESSAGE("test remove by address");
  TEST_ASSERT_TRUE_MESSAGE(linkedList.remove(&hAComponent2),
                           "should be able to mid items");
  TEST_ASSERT_EQUAL_INT(2, linkedList.getSize());
  item = linkedList.get(0);
  TEST_ASSERT_EQUAL_PTR_MESSAGE(&hAComponent1, item,
                                "hAComponent1 should be at position 0");
  item = linkedList.get(1);
  TEST_ASSERT_EQUAL_PTR_MESSAGE(&hAComponent3, item,
                                "hAComponent3 should be at position 1");

  TEST_ASSERT_TRUE_MESSAGE(linkedList.append(&hAComponent2),
                           "append hAComponent2 should return true");
  TEST_ASSERT_TRUE_MESSAGE(linkedList.remove(&hAComponent2),
                           "should be able last item");
  TEST_ASSERT_EQUAL_INT(2, linkedList.getSize());
  item = linkedList.get(0);
  TEST_ASSERT_EQUAL_PTR_MESSAGE(&hAComponent1, item,
                                "hAComponent1 should be at position 0");
  ...
  TEST_ASSERT_TRUE_MESSAGE(linkedList.append(&hAComponent2),
                           "append hAComponent2 should return true");
  TEST_ASSERT_TRUE_MESSAGE(linkedList.remove(&hAComponent1),
                           "should be able root item");
  TEST_ASSERT_EQUAL_INT(2, linkedList.getSize());
  item = linkedList.get(0);
  TEST_ASSERT_EQUAL_PTR_MESSAGE(&hAComponent3, item,
                                "hAComponent3 should be at position 0");
  item = linkedList.get(1);
  TEST_ASSERT_EQUAL_PTR_MESSAGE(&hAComponent2, item,
                                "hAComponent2 should be at position 1");

  // test remove by content
  TEST_MESSAGE("test remove by contant");
  TEST_ASSERT_TRUE_MESSAGE(linkedList.append(&hAComponent1),
                           "append hAComponent2 should return true");
  TEST_ASSERT_TRUE_MESSAGE(linkedList.remove(&hAComponent10, true),
                           "should be able root item");
  TEST_ASSERT_EQUAL_INT(2, linkedList.getSize());
  item = linkedList.get(0);
  TEST_ASSERT_EQUAL_PTR_MESSAGE(&hAComponent3, item,
...
void UT_HANode_constructor(void) {
  ...
  char node1DeviceInfo[] =
      "{\"mf\":\"MANUF\",\"mdl\":\"MODEL\",\"sw\":\"RELEASE\",\"name\":"
      "\"DEVICE1\",\"ids\":\"XXXXXXXX\"}";
  char node2DeviceInfo[] = "{\"mf\":\"MANUF\",\"mdl\":\"MODEL\",\"name\":"
                           "\"DEVICE2\",\"ids\":\"XXXXXXXX\"}";
  ...
  TEST_ASSERT_EQUAL_STRING_MESSAGE("DEVICE1", node1Name,
                                   "Extern data modified by constructor");
  TEST_ASSERT_EQUAL_STRING_MESSAGE("DEVICE2", node2Name,
                                   "Extern data modified by constructor");
  TEST_ASSERT_EQUAL_STRING_MESSAGE("DEVICE1", pNode->getName(),
                                   "Wrong HANode name");
  TEST_ASSERT_EQUAL_STRING_MESSAGE("DEVICE2", pNode2->getName(),
                                   "Wrong HANode name");

  TEST_MESSAGE("test generated id");
  TEST_ASSERT_NOT_EQUAL_MESSAGE(pNode->getId(), pNode2->getId(),
...
void UT_HAComponentProperty(void) {
  ...
  HAComponentProperty *pProperty1 =
      new HAComponentProperty(PROP_STATE_TOPIC, "/state");
  HAComponentProperty *pProperty2 =
      new HAComponentProperty(PROP_STATE_TOPIC, "/toto");
  HAComponentProperty *pProperty3 =
      new HAComponentProperty(PROP_STATE_ON, "/toto");

  TEST_MESSAGE("Test getName");
  // TEST_MESSAGE(pProperty1->getName());
  //  TEST_ASSERT_EQUAL_STRING_MESSAGE(DiscoveryMessageKeyLabel[PROP_STATE_TOPIC],pProperty1->getName(),"wrong
  //  name");
  //  TEST_ASSERT_EQUAL_STRING_MESSAGE(DiscoveryMessageKeyLabel[PROP_STATE_TOPIC],pProperty2->getName(),"wrong
  //  name");
  //  TEST_ASSERT_EQUAL_STRING_MESSAGE(DiscoveryMessageKeyLabel[PROP_STATE_ON],pProperty3->getName(),"wrong
  //  name");

  TEST_MESSAGE("Test getKey");
  // TEST_ASSERT_EACH_EQUAL_INT(PROP_STATE_TOPIC,pProperty1->getKey(),"wrong
  // key");

  TEST_MESSAGE("Test getValue");
  TEST_ASSERT_EQUAL_STRING_MESSAGE("/state", pProperty1->getValue(),
                                   "wrong value");
  TEST_ASSERT_EQUAL_STRING_MESSAGE("/toto", pProperty2->getValue(),
                                   "wrong value");
  TEST_ASSERT_EQUAL_STRING_MESSAGE("/toto", pProperty3->getValue(),
                                   "wrong value");

  TEST_MESSAGE("Test operators");
  TEST_ASSERT_TRUE_MESSAGE((*pProperty1 == *pProperty2),
                           "== operator should be true");
  TEST_ASSERT_FALSE_MESSAGE((*pProperty1 == *pProperty3),
                            "== operator should be false");
  TEST_ASSERT_FALSE_MESSAGE((*pProperty1 != *pProperty2),
                            "== operator should be true");
  TEST_ASSERT_TRUE_MESSAGE((*pProperty1 != *pProperty3),
                           "== operator should be false");
...
void UT_HAComponent_constructor(void) {
  ...
  TEST_ASSERT_EQUAL_STRING_MESSAGE("DEVICE1", component1Name,
                                   "Extern data modified by constructor");
  TEST_ASSERT_EQUAL_STRING_MESSAGE("DEVICE1", pHAComponent1->getName(),
                                   "Wrong HAComponent name");
  TEST_ASSERT_NOT_EQUAL_MESSAGE(pHAComponent1->getId(), pHAComponent2->getId(),
  ...
  TEST_ASSERT_EQUAL_STRING_MESSAGE(
      refTopic, pHAComponent1->getPropertyValue(PROP_ROOT_TOPIC),
      "wrong property ~");

  TEST_MESSAGE("Test default value (PROGMEM access)");
  HAComponentProperty *pPropertyDefault =
      new HAComponentProperty(PROP_AUX_COMMAND_TOPIC);
  TEST_ASSERT_NOT_NULL(pPropertyDefault->getValue());
  // TEST_ASSERT_EQUAL_STRING_MESSAGE("~/auxcmd", pPropertyDefault->getValue(),
  // "wrong property default value");
...
void UT_HAComponentDeviceTrigger(void) {
  ...
  uint32_t freeBefore = ESP.getFreeHeap();
  TEST_MESSAGE("test constructor");
  HAComponentDeviceTrigger *pDeviceTriger =
      new HAComponentDeviceTrigger(component1Name);
...
void UT_HAComponent_buildDiscoveryTopic(void) {
  ...
  char *topic =
      pHAComponent1->buildDiscoveryTopic("discovery_prefix", "portal");
  // TEST_MESSAGE(topic);
  ...
  TEST_ASSERT_EQUAL_STRING_MESSAGE(referenceId, topic + strlen(topic) - 8,
                                   "wrong deviceid in topic");
  delete referenceId;

  topic[strlen(topic) - 8] = 0;
  // TEST_MESSAGE(topic);
  TEST_ASSERT_EQUAL_STRING_MESSAGE(
      "discovery_prefix/binary_sensor/portal/DEVICE1-", topic,
      "postfix of deviceid not the device name");
...
void UT_HAComponent_GetTopicPath(void) {
  ...
  char component1Name[] = "DEVICE1";
  char expextedTopic[] =
      "homeassistant/binary_sensor/testnode-DEVICE1-XXXXXXXX/osccmd";
...
void UT_HAUtils_generateId(void) {
  ...
  TEST_ASSERT_FALSE_MESSAGE(id10 == id103,
                            "Pompes_instant with different type");
...
void resetCallbackCall() { callbackCalled = 0; }
void callback() { callbackCalled++; }
void basicLigthPowerOn(boolean powerOn) { callbackCalled++; }

#define HA_CALLBACK std::function<void()>

void UT_HAComponent_DiscoveryMessage(void) {
  char component1Name[] = "DEVICE1";
  char refDisconeryMsgTemplate[] =
      "{\"name\":\"DEVICE1\",\"uniq_id\":\"%08X\",\"~\":\"homeassistant/"
      "binary_sensor/"
      "portal-DEVICE1-%08X\",\"cmd_off_tpl\":\"OFF\",\"cmd_on_tpl\":\"ON\","
      "\"cmd_t\":\"~/cmd\",\"stat_t\":\"homeassistant/toto\"}";

  uint32_t freeBefore = ESP.getFreeHeap();

  HAComponent *pHAComponent1 = new HAComponent(component1Name, BINARY_SENSOR);
  char *refDisconeryMsg =
      (char *)malloc(strlen(refDisconeryMsgTemplate) - 4 + 8 - 4 + 8 + 1);
  sprintf(refDisconeryMsg, refDisconeryMsgTemplate, pHAComponent1->getId(),
          pHAComponent1->getId());
  HANode *pNode = new HANode("portal");
  pHAComponent1->setNode(pNode);

  HAComponentPropertyAction<HA_CALLBACK> *l_pAction =
      new HAComponentPropertyAction<HA_CALLBACK>(PROP_COMMAND_TOPIC, callback);
  pHAComponent1->addAction(l_pAction);
  l_pAction = new HAComponentPropertyAction<HA_CALLBACK>(
      PROP_STATE_TOPIC, callback, "homeassistant/toto");
  pHAComponent1->addAction(l_pAction);
...
void UT_OnHAMessage(void) {
  ...
  HAComponent *pHAComponent3 = new HAComponentLigthBasic(
      "DEVICE3", [](boolean powerOn) { callbackCalled++; });
  pNode->addComponent(pHAComponent3);

  resetCallbackCall();
  TEST_ASSERT_FALSE(pNode->onHAMessage("toto", NULL, (unsigned int)0));
  TEST_ASSERT_EQUAL_INT_MESSAGE(0, callbackCalled,
                                "No callback should ben called");
  ...
  TEST_ASSERT_TRUE_MESSAGE(pNode->onHAMessage(mqttTopic, (const byte *)"ON",
                                              ((const unsigned int)2)),
                           "action topic do not work");
  delete mqttTopic;
  TEST_ASSERT_FALSE(pNode->onHAMessage("ezr/zer/zer", (const byte *)"ON",
                                       ((const unsigned int)2)));
  TEST_ASSERT_EQUAL_INT_MESSAGE(1, callbackCalled,
                                "Only 1 callback should ben called");
...
void UT_HAAdapter_constructor(void) {
  ...
  pAdapter = new HAAdapterDDS238(
      "MyButton", D2, 1000, 220, 20,
      [pDSS238PesistantMock](DDS238Data data) { pDSS238PesistantMock->save(); },
      true);
...
void UC_SETUP(void) {
  ...
  p_MyDevice->setup("localhost", 8383);
...
public:
  mockHAAdapterLed(const char *name, uint8_t ioReference)
      : HAAdapterLed(name, ioReference) {
    m_poweron = false;
  }
  boolean m_poweron = false;
  virtual void powerOn(boolean powerOn) { m_poweron = powerOn; };
};
...
void UC_StreamHAMessages(void) {
  ...
  p_MyDevice->setup("localhost", 8383); // Does not actually connect without loop()
  
  HAAdapterPushBtn *p_Adapter = new HAAdapterPushBtn("MyButton", D1);
  p_Adapter->setup();
  p_Adapter->setDevice(p_MyDevice);

  HANode* node = p_MyDevice->getNode();

  // On internal connection (virtual), force Discovery generation
  node->onHAConnect(true);

  TEST_MESSAGE("Checking pending Discovery messages");
  TEST_ASSERT_EQUAL_MESSAGE(1, node->actionsSize(), "Must contain Discovery message");

  // Clear initialization actions for the test
  while(node->actionsSize() > 0) {
      HAAction* a = node->pickupAction();
      delete a;
  }

  // Simulate button press
  TEST_MESSAGE("Simulating BTN press");
  p_Adapter->onBtPressed();
  
  // The button adds an event to the queue
  TEST_ASSERT_EQUAL_MESSAGE(1, node->actionsSize(), "An action must be pending after press!");
  
  HAAction* action = node->pickupAction();
  TEST_ASSERT_NOT_NULL_MESSAGE(action, "An action must be generated");
  TEST_ASSERT_EQUAL_INT_MESSAGE(POSTMESSAGE, action->getType(), "It should be a PUBLISH");
  
  HAMessage* pubMsg = (HAMessage*)action;
  TEST_ASSERT_EQUAL_STRING("1", pubMsg->getMessage());
...
void UT_HAComponentTextSensor(void) {
  char componentName[] = "TEXT_SENSOR1";
  uint32_t freeBefore = ESP.getFreeHeap();

  HAComponentTextSensor *pTextSensor = new HAComponentTextSensor(componentName);

  // Test constructor
  TEST_ASSERT_EQUAL_STRING_MESSAGE("TEXT_SENSOR1", pTextSensor->getName(),
                                   "Wrong component name");

  HANode *pNode = new HANode("portal");
  pTextSensor->setNode(pNode);

  // Test setting text value
  pTextSensor->setValue("Hello");
  pTextSensor->setValue(String("HA"));
  ...
  delay(10);
  uint32_t freeAfter = ESP.getFreeHeap();
  TEST_ASSERT_INT32_WITHIN_MESSAGE(1000, freeBefore, freeAfter, "Memory leak");
}
...
void setup() {
  ...
  RUN_TEST(UT_HAComponentTextSensor);
  RUN_TEST(UC_SETUP);
